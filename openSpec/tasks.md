...
- [ ] 0.6 Initial commit; create GitHub repo; connect Vercel early (deploy the skeleton now — de-risks Phase 8)

## Phase 1 — Database Layer (30 min)

- [ ] 1.1 `packages/db`: Drizzle + `@neondatabase/serverless`; define `users`, `documents`, `document_shares` per design.md §5
- [ ] 1.2 Generate + run first migration against Neon
- [ ] 1.3 Seed script: creates Alice + Bob in Firebase (Admin SDK) and Postgres; creates one sample doc owned by Alice, shared to Bob as editor
...
- [ ] 3.1 `getDocumentAccess(docId, userId)` helper (owner → shares lookup)
...
- [ ] 4.1 Firebase client SDK setup; auth context/hook exposing user + `getIdToken()`
- [ ] 4.2 Typed API client (fetch wrapper attaching Bearer token) + TanStack Query provider
...
- [ ] 6.1 `POST /import`: multipart, accept `.txt`/`.md` ≤ 1 MB, reject otherwise with clear 400
- [ ] 6.2 Convert: `.md` → marked → `generateJSON` (Tiptap html util) → jsonb; `.txt` → paragraph nodes; title from filename
