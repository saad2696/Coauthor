...
- [x] 7.2 Share dialog in editor header: email input, role select (viewer/editor), collaborator list with revoke
- [x] 7.3 End-to-end manual check: Alice shares → Bob sees in "Shared with me" → Bob (editor) edits → Alice sees update; viewer role is read-only
...
- [ ] 8.5 (deferred) Production deploy: env vars on Vercel, migrate + seed prod Neon, full smoke test of the reviewer flow on live URL — DEPLOY.md written; deploy deferred per owner (needs Vercel login). Prod reuses the shared Neon DB (already migrated + seeded).
...
- [x] 9.4 SUBMISSION.md: contents list, live URL, credentials, what works / what's partial / next 2–4 hours
- [ ] 9.5 (owner action) Record 3–5 min Loom: user flow → what works E2E → deliberate cuts → key decisions (D1, D6, D8) → AI workflow
- [ ] 9.6 (owner action) Google Drive folder: code zip/link, README, architecture note, AI note, SUBMISSION.md, video-url.txt, screenshots/GIF
