...
> **D4 amendment (owner-approved, post-Phase-7):** Signup no longer takes a password on the frontend. The signup form collects **name + email** and POSTs to a public `POST /api/auth/register`, which uses the **Firebase Admin SDK** to create the account with a **backend-generated random password** + display name and mirror it into `users`. The frontend then triggers Firebase's built-in **password-reset email** (`sendPasswordResetEmail`) so the user sets their own password; they then log in normally. **Login remains email/password**, so seeded reviewer accounts (Alice/Bob) still work instantly. No SMTP needed (Firebase sends the email). Token verification / user-mirroring (the core of D4) is unchanged. Note: `/api/auth/register` is intentionally public (pre-auth); for production it would want rate-limiting / CAPTCHA.
